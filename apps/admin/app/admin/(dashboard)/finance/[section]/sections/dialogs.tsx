...
export function CreateInvoiceDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  // Prefill TC con blue.venta cuando currency cambia a ARS; limpia al volver a USD
  useEffect(() => {
  ...
  // Preview del subtotal en USD cuando la factura está en ARS
  const subtotalBase = useMemo(() => {
    if (watchedCurrency === 'USD') return null
    if (watchedCurrency === 'ARS' && watchedRate) return subtotal / watchedRate
    return null
  }, [subtotal, watchedCurrency, watchedRate])
        ...
        <Form {...form}>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                <Label>Deal / Proyecto (recomendado)</Label>
                <Controller
                  control={control}
                  name="dealId"
                  render={({ field }) => (
                ...
                />
                {/* Aviso cuando no hay deal: la factura no aparece en Rentabilidad por proyecto */}
                {!watch('dealId') && (
                  <p className="text-xs text-amber-600 dark:text-amber-400">
                    Sin deal, esta factura no va a aparecer en Rentabilidad por proyecto.
                  </p>
                )}
              </div>
            </div>
            ...
            {watchedCurrency === 'ARS' && (
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  <Label htmlFor="inv-rate">Tipo de cambio (ARS por USD)</Label>
                  {fx && (
                ...
                {subtotalBase != null && (
                  <p className="text-xs text-muted-foreground">
                    ≈ {formatCurrency(subtotalBase, 'USD')} USD
                  </p>
                )}
